...
PARSE_API_KEY = "pmx_026c6b8f9111ae88203815e575efc17c"  # <-- ЗАМЕНИТЕ
...
def parse_message_fruits(text):
    fruits = set()
    if not text: return fruits
    for line in text.splitlines():
        m = re.match(r"^\d+\.\s*(.+)$", line.strip())
        if m: fruits.add(m.group(1).strip())
    return fruits
...
def cleanup_duplicates():
    ...
    for msg in messages:
        ...
        if text in seen: to_delete.append(msg["id"])
        else: seen[text] = msg["id"]
    if to_delete:
        try:
            for i in range(0, len(to_delete), 100):
                chunk = to_delete[i:i+100]
                vk.messages.delete(message_ids=",".join(str(x) for x in chunk), delete_for_all=1)
    ...
    for msg in messages:
        ...
        if text.startswith("🍎 ОБЫЧНЫЙ СТОК") and last_normal_text is None: last_normal_text = text
        elif text.startswith("✨ МИРАЖНЫЙ СТОК") and last_mirage_text is None: last_mirage_text = text
        if last_normal_text and last_mirage_text: break
...
def bot_loop():
    ...
    if last_normal_text: print("[Startup] Последний Normal уже в истории.")
    if last_mirage_text: print("[Startup] Последний Mirage уже в истории.")
    while True:
        data = get_stock()
        if data:
